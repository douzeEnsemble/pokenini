...
class CalculateGameBundlesShiniesAvailabilitiesCommandTest extends TestCase
{
    public function testFailureOnException(): void
    {
        ...
        $actionStarter = new CalculateGameBundlesShiniesAvailabilitiesActionStarter($entityManager);

        $calculatorService = $this->createMock(GameBundlesShiniesAvailabilitiesCalculatorService::class);
        $calculatorService
            ->expects($this->once())
            ->method('execute')
        ...
        $command = new CalculateGameBundlesShiniesAvailabilitiesCommand(
            $translator,
            $entityManager,
            $actionStarter,
            $calculatorService,
