...
class CalculateGameBundlesAvailabilitiesCommandTest extends TestCase
{
    public function testFailureOnException(): void
    {
        $translator = $this->createMock(TranslatorInterface::class);

        $actionLog = new ActionLog('CalculateGameBundlesAvailabilities');
        ...
        $actionStarter = new CalculateGameBundlesAvailabilitiesActionStarter($entityManager);

        $calculatorService = $this->createMock(GameBundlesAvailabilitiesCalculatorService::class);
        $calculatorService
            ->expects($this->once())
            ->method('execute')
        ...
        $command = new CalculateGameBundlesAvailabilitiesCommand(
            $translator,
            $entityManager,
            $actionStarter,
            $calculatorService,
