...
use App\Web\Controller\AlbumDexController;
...
class ALbumDexTest extends WebTestCase
{
    ...
    public function testHomeAsAdmin(): void
    {
        ...
        $thirdAlbum = $crawler->filter('.dex-item')->eq(3);
    ...
    public function testNonConnectedHome(): void
    {
        ...
        $this->assertCountFilter($crawler, 8, '.dex-item');
        $this->assertCountFilter($crawler, 8, '.dex-item h5');
        $this->assertCountFilter($crawler, 1, '.dex-item h6');
        ...
        for ($i = 0; $i < 7; ++$i) {
            ...
            $this->assertEquals(
                ...
                $crawler->filter('.dex-item')->eq($i)->filter('a')->attr('href')
            );
            $this->assertEquals(
                "https://icon.pokenini.fr/banner/{$dexSlug}.png",
                $crawler->filter('.dex-item')->eq($i)->filter('img')->attr('src')
            );
        }
    }
