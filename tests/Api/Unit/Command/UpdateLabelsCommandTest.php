...
class UpdateLabelsCommandTest extends TestCase
{
    public function testFailureOnException(): void
    {
        $translator = $this->createMock(TranslatorInterface::class);

        $actionLog = new ActionLog('UpdateLabels');
        ...
        $actionStarter = new UpdateLabelsActionStarter($entityManager);

        $updaterService = $this->createMock(LabelsUpdaterService::class);
        $updaterService
            ->expects($this->once())
            ->method('execute')
        ...
        $command = new UpdateLabelsCommand(
            $translator,
            $entityManager,
            $actionStarter,
            $updaterService,
